...
export default function EditDeleteRequest(props) {
  let { classId, projectId, taskId, getTaskFunc, updateTaskFunc } = props;
  ...
  const postUpdatedTask = () => {
    ...
    let fetchUrl = `/v1/classes/${classId}/projects/${projectId}/tasks/${taskId}`;
    let fetchOptions = {
      method: 'POST', 
      headers: {'Content-Type':'application/json', 'Authorization': authHdr.Authorization},
      body: JSON.stringify(
          {shortDescr: shortDescr, longDescr: longDescr, expectedDate: expectedDateStr})
    }
  ...
  useEffect(() => {
    ...
    setShortDescr(initTask.shortDescr);
    setLongDescr(initTask.longDescr);
